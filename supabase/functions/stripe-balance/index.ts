...
serve(async (req) => {
  ...
  try {
    ...
    // Handle ping request to check API connectivity
    if (reqBody.ping === true) {
      console.log("Stripe API ping request received");
      
      // Check if we have a Stripe key configured
      const stripeKey = Deno.env.get("STRIPE_SECRET_KEY");
      if (!stripeKey) {
        console.error("Stripe API key not configured when handling ping request");
        return new Response(
          JSON.stringify({ 
      ...
      // Return a successful ping response without making actual API calls
      return new Response(
        JSON.stringify({ 
          status: 'success', 
          ping: true,
          message: 'Stripe API is configured',
          config_status: 'ready'
        }),
        { 
          status: 200, 
    ...
    // For normal data requests, extract the date range
    const { startDate, endDate, forceRefresh, rawResponse } = reqBody;

    // Validate inputs for data requests
    if (!startDate || !endDate) {
      ...
      return new Response(
        JSON.stringify({ 
          ...
          status: 'error',
          errorDetails: { startDateProvided: !!startDate, endDateProvided: !!endDate }
        }),
        { 
          status: 400, 
          headers: { ...corsHeaders, "Content-Type": "application/json" } 
    ...
    const stripeKey = Deno.env.get("STRIPE_SECRET_KEY");
    if (!stripeKey) {
      console.error("Stripe API key not configured when handling data request");
      return new Response(
        JSON.stringify({ 
          error: "Stripe API key not configured",
          status: 'error',
    ...
    try {
      // Try to initialize Stripe client to validate the key format
      ...
      while (hasMore) {
        ...
        try {
          ...
          console.error("Error in Stripe API call:", apiError);
          return new Response(
            JSON.stringify({ 
              error: apiError.message || "Failed to fetch Stripe data during pagination",
              status: 'error',
              errorDetails: {
                type: apiError.type || 'unknown',
                code: apiError.statusCode || 'unknown',
                param: apiError.param || 'unknown'
              }
            }),
            { 
              status: 500, 
      ...
      // If raw response is requested, return the raw transaction data with minimal processing
      if (rawResponse) {
        return new Response(
          JSON.stringify({
      ...
      return new Response(
        JSON.stringify({ 
          error: stripeError.message || "Failed to initialize Stripe client",
          status: 'error',
          errorDetails: {
            type: stripeError.type || 'unknown',
            apiKeyLength: stripeKey ? `${stripeKey.length} characters` : 'missing',
            apiKeyValid: stripeKey ? (stripeKey.startsWith('sk_') ? 'format_valid' : 'format_invalid') : 'missing'
          }
        }),
        { 
          status: 500, 
