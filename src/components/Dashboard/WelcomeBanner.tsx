...
const WelcomeBanner: React.FC<WelcomeBannerProps> = ({ handleInitialLoad, isLoading }) => {
  return (
    <div className="flex flex-col items-center justify-center py-16 bg-white rounded-lg shadow-sm mb-6">
      ...
      <Button onClick={handleInitialLoad} className="gap-2" disabled={isLoading}>
        {isLoading ? (
          <>
            <Loader className="h-4 w-4 animate-spin" /> Cargando datos...
          </>
        ) : (
          <>
