...
- **Sortable table** — click any column header (code, name, grade, status, location, paid) to
  sort; click again to reverse.
- **Export** the current (filtered) view to **CSV** (spreadsheet) or **JSON** (backup).
- "Reset" restores the original data shipped from the source sheet.

## Storage: local JSON file

Data lives in a **plain JSON file on your machine** (`~/Documents/Gunpla Inventory/
inventory.json`), served by a tiny Node API in [`/server`](../../../server). See
[`server/README.md`](../../../server/README.md) to start it.

- **Server running** → the JSON file is the source of truth. The header shows a green
  **Local file** badge. A `localStorage` copy is kept as an offline mirror.
...
├── hooks/useGunplaInventory.ts  # JSON-file API w/ localStorage fallback
├── lib/
│   ├── inventory.ts          # filtering, stats, helpers (pure)
│   ├── api.ts                # local JSON-file server client
...
- The JSON file stays on your machine — nothing is sent to any cloud service.
